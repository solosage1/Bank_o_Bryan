...
import { getBrowserTimeZone, getTimezoneLabel } from '@/lib/time';
  ...
  const createAccountForChild = async (childId: string, childName: string) => {
    try {
      const { error } = await supabaseWithTimeout(
        async () => supabase.from('accounts').insert({ child_id: childId }),
        6000
      );
      if ((error as any)?.error) throw (error as any).error;
